    ...
    return (
      <div className="min-h-screen max-w-md mx-auto">
        <div
          ...
          style={{
            background: 'radial-gradient(ellipse at 20% 0%, rgba(124,58,237,0.05) 0%, transparent 60%), radial-gradient(ellipse at 80% 100%, rgba(219,39,119,0.04) 0%, transparent 60%)',
          }}
        />
  ...
  return (
    <div className="min-h-screen max-w-md mx-auto relative">
      <div
        className="fixed inset-0 pointer-events-none"
        style={{
          background: 'radial-gradient(ellipse at 70% 0%, rgba(124,58,237,0.07) 0%, transparent 50%), radial-gradient(ellipse at 10% 80%, rgba(219,39,119,0.05) 0%, transparent 50%)',
        }}
      />

      <div className="sticky top-0 z-20 glass border-b border-slate-200/80 px-4 pt-12 pb-3">
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">Журнал жалоб</p>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={() => setSortBy(s => s === 'date' ? 'priority' : 'date')}
              className="w-9 h-9 bg-slate-100 rounded-xl flex items-center justify-center transition-all hover:bg-slate-200"
            >
              <Icon name={sortBy === 'date' ? 'Clock' : 'ArrowUpDown'} size={16} className="text-slate-400" />
            </button>
            <button
              onClick={() => setShowFilters(f => !f)}
              className="w-9 h-9 rounded-xl flex items-center justify-center transition-all relative"
              style={{
                background: showFilters ? 'rgba(124,58,237,0.12)' : '#f1f0f7',
                border: showFilters ? '1px solid rgba(124,58,237,0.3)' : '1px solid transparent',
              }}
            >
              <Icon name="SlidersHorizontal" size={16} style={{ color: showFilters ? '#7c3aed' : '#94a3b8' }} />
              {hasActiveFilters && (
                <span
                  className="absolute -top-0.5 -right-0.5 w-2 h-2 rounded-full"
                  style={{ background: '#7c3aed', boxShadow: '0 0 5px rgba(124,58,237,0.5)' }}
                />
              )}
            </button>
          </div>
        ...
        <div className="relative mb-3">
          <Icon name="Search" size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-300 pointer-events-none" />
          <input
            type="text"
            value={searchQuery}
            onChange={e => setSearchQuery(e.target.value)}
            placeholder="Поиск по жалобам..."
            className="w-full bg-slate-100 border border-slate-200 rounded-xl pl-9 pr-4 py-2.5 text-sm text-slate-700 placeholder-slate-300 outline-none focus:border-violet-400 transition-colors"
          />
          {searchQuery && (
            <button onClick={() => setSearchQuery('')} className="absolute right-3 top-1/2 -translate-y-1/2">
              <Icon name="X" size={14} className="text-slate-300" />
            </button>
          )}
        </div>
          ...
          {(['all', 'new', 'in_review', 'resolved', 'rejected'] as FilterStatus[]).map((s) => {
            ...
            return (
              <button
                ...
                style={{
                  background: active ? (cfg ? cfg.bg : 'rgba(124,58,237,0.1)') : '#f1f0f7',
                  color: active ? (cfg ? cfg.color : '#7c3aed') : '#94a3b8',
                  border: active ? `1px solid ${cfg ? cfg.color + '35' : 'rgba(124,58,237,0.25)'}` : '1px solid transparent',
                  boxShadow: active && cfg ? `0 2px 8px ${cfg.glow}` : 'none',
                }}
              >
                {isAll ? 'Все' : cfg!.label}
                <span
        ...
        {showFilters && (
          <div className="mt-3 space-y-2 animate-fade-in">
            <div>
              <p className="text-xs text-slate-400 mb-1.5">Тип нарушения</p>
                ...
                {(['all', 'spam', 'abuse', 'fraud', 'illegal', 'misinformation', 'other'] as FilterType[]).map((t) => {
                  ...
                  return (
                    <button
                      ...
                      style={{
                        background: active ? 'rgba(124,58,237,0.1)' : '#f1f0f7',
            ...
            <div>
              <p className="text-xs text-slate-400 mb-1.5">Приоритет</p>
                  ...
                  return (
                    <button
                      ...
                      style={{
                        background: active ? (cfg ? cfg.bg : 'rgba(124,58,237,0.1)') : '#f1f0f7',
                        color: active ? (cfg ? cfg.color : '#7c3aed') : '#94a3b8',
                        border: active ? `1px solid ${cfg ? cfg.color + '30' : 'rgba(124,58,237,0.25)'}` : '1px solid transparent',
                      }}
                    >
                      {p === 'all' ? 'Все' : cfg!.label}
                    </button>
      ...
      <div className="px-4 py-3">
        ...
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 text-center animate-fade-in">
            ...
            <p className="text-xs text-slate-300 mt-1">Попробуйте изменить фильтры</p>
          </div>
        ) : (
